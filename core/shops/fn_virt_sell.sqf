...
if(!([_amount] call fnc_isnumber)) exitWith {hint "Tu n'as pas rentre un bon chiffre!";};
_amount = parseNumber (_amount);
if(_amount > (missionNameSpace getVariable _var)) exitWith {hint "Tu en a pas autant!"};
    
_price = (_price * _amount);
_name = [_var] call life_fnc_vartostr;
if(([false,_type,_amount] call life_fnc_handleInv)) then
{
    hint format["Tu as vendus %1 %2 pour $%3",_amount,_name,[_price] call life_fnc_numberText];
