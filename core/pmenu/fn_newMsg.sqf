...
ctrlSetText[88886, format["Message de: %1",name life_smartphoneTarget]];
...
case 1:
{
if(isNUll life_smartphoneTarget) exitWith {hint format["Aucun selectionné !"];};
ctrlShow[88885, false];
if(_msg == "") exitWith {hint "Vous devez écrire quelque chose";ctrlShow[88885, true];};
[[life_smartphoneTarget,_msg,player,0],"TON_fnc_handleMessages",false] spawn life_fnc_MP;
hint format["Message à %1 : %2",name life_smartphoneTarget,_msg];
ctrlShow[88885, true];
closeDialog 88883;
};
//copmessage
...
_to = "Police";
hint format["Message à %1 : %2",_to,_msg];
...
if(_msg == "") exitWith {hint "Vous devez écrire quelque chose !";ctrlShow[888896,true];};
...
case 4:
{
if(({side _x == independent} count playableUnits) == 0) exitWith {hint format["Actuellement aucun membre de la sécurité civile est en service. Veuillez réessayer plus tard"];};
ctrlShow[888899,false];
if(_msg == "") exitWith {hint "Vous devez écrire quelque chose !";ctrlShow[888899,true];};
[[ObjNull,_msg,player,3],"TON_fnc_handleMessages",false] spawn life_fnc_MP;
hint format["Vous avez envoyé un message a la sécurité civile.",_msg];
...
if(isNULL life_smartphoneTarget) exitWith {hint format["Merci de choisir une personne !"];};
...
hint format["Admin message envoyé à tous les joueurs: %1",_msg];
closeDialog 887890;
};
