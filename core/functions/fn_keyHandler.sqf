...
switch (_code) do
{
		...
		if([false,"zipties",1] call life_fnc_handleInv) then
    {
			...
			hint "Tu l'as mennotte, WINDOWS pour le menu";
    }
		else
	{
			hint "Tu n'as pas de zipties!";
		};
	};
};			
    ...
    case 34:
    {    
        if(!_shift && !_ctrlKey && !_alt && (!life_action_inUse) && (vehicle player == player) ) then
        {
            {
	...
	case _interactionKey:
	{
		if(!life_action_inUse) then {
			...
			{
				player setObjectTextureGlobal [0, "textures\Gendarme\range_gendarme.jpg"];
			};
			...
			{
				player setObjectTextureGlobal [0, "textures\Gendarme\gendarme_bleu.paa"];
			};
			if(playerSide == west && uniform player == "U_B_CombatUniform_mcam_vest") then
			{
				player setObjectTextureGlobal [0, "textures\Gendarme\gendarme_noir.paa"];
			};
			if(playerSide == west && uniform player == "U_B_CTRG_1") then
			{
			...
			if(playerSide == civilian && uniform player == "U_C_Poloshirt_burgundy") then
			{
				player setObjectTextureGlobal [0, "textures\Civil\BananaWar.jpg"];
			};
			if(playerSide == civilian && uniform player == "U_C_Poloshirt_redwhite") then
			{
				player setObjectTextureGlobal [0, "textures\Civil\BB_Danger.jpg"];
			};
			if(playerSide == civilian && uniform player == "U_C_Poloshirt_salmon") then
			{
				player setObjectTextureGlobal [0, "textures\Civil\BewarOfBiters.jpg"];
			};
			if(playerSide == civilian && uniform player == "U_C_Poloshirt_stripped") then
			{
				player setObjectTextureGlobal [0, "textures\Civil\fut1.jpg"];
			};
			if(playerSide == civilian && uniform player == "U_C_Poloshirt_tricolour") then
			{
				player setObjectTextureGlobal [0, "textures\Civil\FUT2.jpg"];
			};
			if(playerSide == civilian && uniform player == "U_C_Commoner1_1") then
			{
				player setObjectTextureGlobal [0, "textures\Civil\NightWatch.jpg"];
			};
			if(playerSide == civilian && uniform player == "U_C_Commoner1_2") then
			{
			...
			{
				(unitbackpack player) setObjectTextureGlobal [0, "textures\Gendarme\backpack_gendarmerie.jpg"];
			};
			if (Backpack player == "B_Bergen_blk" && playerSide == west) then
			{
				(unitbackpack player) setObjectTextureGlobal [0, "textures\Gendarme\backpack_gign.jpg"];
			};
			...
			{
				(unitbackpack player) setObjectTextureGlobal [0, "textures\Securite\backpack_securite.paa"];
			};
		};
	};
	...
	//Vol
        if(_shift && playerSide == civilian && !isNull cursorTarget && cursorTarget isKindOf "Man" && isPlayer cursorTarget && alive cursorTarget && cursorTarget distance player < 4 && speed cursorTarget < 1) then
        {
            if((animationState cursorTarget) != "Incapacitated" && (currentWeapon player == primaryWeapon player OR currentWeapon player == handgunWeapon player) && currentWeapon player != "" && !life_knockout && !(player getVariable["restrained",false]) && !life_istazed && !(player getVariable["surrender",false])) then
            {
				...
				hint "Le téléphone de la personne est tombé a terre !";
				_defenceplace1 = "Item_ItemRadio" createVehicle (player modelToWorld[0,0,0]);}
				else { hint "La personne n'avait pas de téléphone sur lui !"};
            };
            _handled = true;
        };
    };	
	...
	case 38: 
	{
		...
		if(_shift && playerSide in [west,independent]) then {
			if(vehicle player != player && (typeOf vehicle player) in ["B_Quadbike_01_F","C_Offroad_01_F","C_SUV_01_F","C_Hatchback_01_F","C_Hatchback_01_sport_F","C_Van_01_box_F","C_Heli_Light_01_civil_F","B_MRAP_01_F","I_MRAP_03_F","I_MRAP_03_hmg_F","B_Heli_Light_01_F"]) then {
			...
			if(_veh isKindOf "House_F" && playerSide == civilian) then {
				...
				if(_veh in life_vehicles && player distance _veh < 8) then {
					if(_locked == 2) then {
						...
						hint composeText [ image "icons\lock.paa", " Vehicule Fermé" ];
						player say3D "car_lock";
